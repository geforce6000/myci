<?php
	...
	class Article extends CI_Controller
	{
		public function id() { //根据articleid调取一条记录并返回给article视图显示
			...
			$res['lastten']=$this->article->getarticlebyclass(97,0,10);
		...
		public function search () { //根据搜索关键字查询记录返回articlefound视图，每20条视图做一次分页
		...
		public function category () { //根据文章的classid来显示相关分类的文章，一页10篇，带分页
			...
			$data=$this->article->getArticlebyClass($articleclass, $startwith, 10); //调用Article_model模型中的getarticlebyclass方法
			$this->load->model('My_model', 'pagi');
			$paginationinfo = array(
...
*/
		...
		public function showChildCategory () { //根据post进来的ID做为父ID，查询到该ID对应的全部子ID返回
		...
		public function showArticleinTable () { //根据POST进来的ID作为classid，查询相关文章10篇显示在table中
